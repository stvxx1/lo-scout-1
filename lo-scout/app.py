...
def fetch_data(p_type, h_range, w_range, a_range, niches, page, p_l, p_t):
    ...
    query_url = (
        f"{base_url}?q=&page={page}&f[performerType]={p_type_val}{cat_query}"
    ...
    if p_type == "Male":
        query_url += f"&r[appearance.metric.penis_length]={p_l[0]},{p_l[1]}"
        query_url += f"&r[appearance.metric.penis_thickness]={p_t[0]},{p_t[1]}"
    ...
    try:
        resp = scraper.get(query_url, headers=headers, timeout=15)
